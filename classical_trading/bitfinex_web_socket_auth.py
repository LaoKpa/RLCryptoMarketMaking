...
class OrderStreamParder(object):
...
class TradeUpdateParder(object):
    def __init__(self, order_request_responsoe):
        self.chanle_id = order_request_responsoe[0]
        self.event_string = order_request_responsoe[1]
    ...
    def get_response_object(self, resp):
        if type(resp) is dict:
            ...
            if 'on' in resp:
                ...
                return TradeEventParder(resp)
            elif 'tu' in resp:
                return TradeUpdateParder(resp)
            elif 'os' in resp:
                return OrderStreamParder(resp)
...
class BitfinexWebSocketClient(threading.Thread):
    ...
    def process_order_stream(self, trade):
        pass
    ...
    def run(self):
        while True:
            ...
            if not resp == BITFINEX_HEART_BEAT:
                ...
                elif type(resp_factory_object) is TradeEventParder:
                    self.trade_event_stack.append(resp_factory_object)
                elif type(resp_factory_object) is TradeUpdateParder:
                    self.trade_update_stack.append(resp_factory_object)
                elif type() is OrderStreamParder:
